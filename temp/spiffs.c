...
spiffs_config spiffs_get_storage_config()
{
	...
	debugf("spiffs: cfg.phys_addr:0x%x \n", cfg.phys_addr);
	debugf("spiffs: INTERNAL_FLASH_SIZE:%d KB\n", INTERNAL_FLASH_SIZE/1024);
...
void spiffs_mount()
{
  ...
  debugf("fs read magic");
  flashmem_read(&dat, cfg.phys_addr, 4);
  debugf("fs magic:0x%X", dat);
